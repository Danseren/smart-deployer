...
abstract contract AutoVestingSplitter is AbstractUtilityContract, Ownable {
    ...
    fallback() external {
        revert("ERC20 transfer not supported");
    }

    function _splitETH(uint256 amount) internal {
        for (uint256 i = 0; i < payees.length; i++) {
            PayeeInfo memory payee = payees[i];
            uint256 payment = (amount * payee.share) / totalShares;
            if (payment > 0) {
                payable(payee.vestingWallet).transfer(payment);
                emit VestedETHForwards(payee.account, payee.vestingWallet, payment);
            }
        }
    }
